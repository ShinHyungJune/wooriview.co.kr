                ...
                <form action="" @submit.prevent="store">
                    <ul class="form-box">
                        <li class="col-group file img">
                            ...
                            <div class="user">
                                <input-images id="pc" :multiple="false" @change="(data) => form.files = data" :default="[item && item.img ? item.img : '']" />
...
export default {
    ...
    methods: {
        store(){
            if(this.item)
                return this.form.post("/api/admin/guides/update/" + this.item.id)
                    .then(response => {
                        this.$router.back();
                    });

            this.form.post("/api/admin/guides")
                .then(response => {
                    this.$router.back();
                });
        },
    ...
    mounted() {
        if(this.$route.query.id){
            return this.$axios.get("/api/admin/guides/" + this.$route.query.id)
                .then(response => {
