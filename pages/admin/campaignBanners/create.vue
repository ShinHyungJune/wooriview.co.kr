                ...
                <form action="" @submit.prevent="store">
                    <ul class="form-box">
                        ...
                        <li class="col-group file img">
                            ...
                            <div class="user">
                                <input-images id="mobile" :multiple="false" @change="(data) => form.files_mobile = data" :default="[item ? item.mobile : '']" />
                                <error :form="form" name="files" />
                            </div>
                        </li>
                        <li class="col-group">
...
export default {
    ...
    methods: {
        store(){
            if(this.item)
                return this.form.post("/api/admin/campaignBanners/update/" + this.item.id)
                    .then(response => {
                        this.$router.back();
                    });

            this.form.post("/api/admin/campaignBanners")
                .then(response => {
                    this.$router.back();
                });
        },
    ...
    mounted() {
        if(this.$route.query.id){
            return this.$axios.get("/api/admin/campaignBanners/" + this.$route.query.id)
                .then(response => {
